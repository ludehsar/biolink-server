...
export const getDismissedReportsPaginated = async (
  ...
  const qb = getRepository(Report)
    ...
    .where(`report.status = '${ResolveStatus.Dismissed}'`)
    .andWhere(
      new Brackets((qb) => {
  ...
  await captureUserActivity(adminUser, context, `Requested dismissed reports`, false)
