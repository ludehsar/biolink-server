...
export const getDiscountCodesPaginated = async (
  ...
  const qb = getRepository(Code)
    ...
    .where(`code.type = '${CodeType.Discount}'`)
    .andWhere(
      new Brackets((qb) => {
  ...
  await captureUserActivity(adminUser, context, `Requested discount codes`, false)
