...
export const getBlackListedUsernamesPaginated = async (
  ...
  const qb = getRepository(BlackList)
    ...
    .where(`blackList.blacklistType = '${BlacklistType.Username}'`)
    .andWhere(
      new Brackets((qb) => {
  ...
  await captureUserActivity(adminUser, context, `Requested blacklisted usernames`, false)
